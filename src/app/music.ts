...
module.exports = function(client: Client, bot: Bot) {
	// plex commands -------------------------------------------------------------
	...
	client.on('messageCreate', function(message: Message){
		...
		if (msg.startsWith(bot.config.caracteres_commande)){
			if(bot.config.canal_ecoute == '' || (message.channel as TextChannel).name == bot.config.canal_ecoute) {
				...
				if(cmdTxt === "?") {
					if(query) {
						...
						if(cmdAide) {
							...
							message.reply({
								content: bot.language.MUSIC_HELP_1.format({caracteres_commande : bot.config.caracteres_commande}),
								tts: true
							});
						//}
						return ;
